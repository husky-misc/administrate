...
describe Administrate::Generators::Views::FieldGenerator, :generator do
  context "for an existing field type" do
    describe "administrate:views:field field_name" do
      it "copies the `_show` partial into the engines/admin/app/views/fields directory" do
        expected_contents = contents_for_field_template(:string, :show)

        run_generator ["string"]
        contents = File.read(file("engines/admin/app/views/fields/string/_show.html.erb"))

        expect(contents).to eq(expected_contents)
      end

      it "copies the `_form` partial into the engines/admin/app/views/fields directory" do
        expected_contents = contents_for_field_template(:string, :form)

        run_generator ["string"]
        contents = File.read(file("engines/admin/app/views/fields/string/_form.html.erb"))

        expect(contents).to eq(expected_contents)
      end

      it "copies the `_index` partial into the engines/admin/app/views/fields directory" do
        expected_contents = contents_for_field_template(:string, :index)

        run_generator ["string"]
        contents = File.read(file("engines/admin/app/views/fields/string/_index.html.erb"))
    ...
    describe "administrate:views:field all" do
      let(:field_types) do
        Dir.entries("engines/admin/app/views/fields").reject { |name| name[0] == "." }
      end
      ...
      it "copies the `_show` partial for each field type" do
        ...
        field_types.each do |field_type|
          ...
          contents = File.read(
            file("engines/admin/app/views/fields/#{field_type}/_show.html.erb"),
          )
      ...
      it "copies the `_form` partial for each field type" do
        ...
        field_types.each do |field_type|
          ...
          contents = File.read(
            file("engines/admin/app/views/fields/#{field_type}/_form.html.erb"),
          )
      ...
      it "copies the `_index` partial for each field type" do
        ...
        field_types.each do |field_type|
          ...
          contents = File.read(
            file("engines/admin/app/views/fields/#{field_type}/_index.html.erb"),
          )
  ...
  def contents_for_field_template(field_name, partial_name)
    File.read(
      "engines/admin/app/views/fields/#{field_name}/_#{partial_name}.html.erb",
    )
  end
end
